...
dQ14 = subset(dQ14, ! Gear %in% c("DT","HAK")) 
...
mymodel ="Quarter+s(Year,Quarter,bs='re') + Gear + s(utm.x,utm.y,bs='ts',k=144,by=Quarter) + ti(ctime,utm.x,utm.y,d=c(1,2),bs=c('ts','ts'),k=c(16,9)) + DepthRS:Quarter + I(DepthRS^2):Quarter  +s(TimeShotHour,k=6,bs='cc')+s(Ship,bs='re') + offset(log(HaulDur))";
